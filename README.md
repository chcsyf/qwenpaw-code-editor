...
- 工具栏：刷新目录 / 新建文件 / 保存
- **从文件浏览器跳转打开**：文件浏览器中点击文件「✏️ 编辑」按钮（或预览弹层「✏️ 编辑」），自动跳转到本插件并打开该文件，可直接编辑保存
...
|---|---|
| 打开文件 | 点击左侧文件树中的文件 |
| 从文件浏览器打开 | 文件浏览器文件行「✏️ 编辑」按钮或预览弹层「✏️ 编辑」 |
| 保存 | `Ctrl+S` 或工具栏 💾 保存 |
...
- **跨插件跳转**：文件浏览器「✏️ 编辑」一键跳转打开指定文件（同源 localStorage 传参，
  SPA 路由会丢弃 URL query 故不用 `?file=` 参数；打开后目标文件自动定位到文件树）
